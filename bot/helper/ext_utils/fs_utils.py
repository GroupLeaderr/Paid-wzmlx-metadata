...
async def edit_metadata(listener, base_dir: str, media_file: str, outfile: str, metadata: str = ''):
    cmd = [bot_cache['pkgs'][2], '-hide_banner', '-ignore_unknown', '-i', media_file, '-metadata', f'title={metadata}',
           '-metadata:s:v', f'title={metadata}', '-metadata', f'Comment= ', '-metadata', f'Copyright= ', '-metadata', f'AUTHOR=MrSagarBots', '-metadata', f'Encoded by= ', '-metadata', f'Encoded_by= ', '-metadata', f'Description= ', '-metadata', f'description= ', '-metadata', f'SUMMARY= ', '-metadata', f'WEBSITE= ', '-metadata:s:a', f'title={metadata}',
